...
class MedGemmaWrapper:
    ...
    def load(self, use_base_model: bool = False):
        """
        Loads MedGemma. 
        ...
        try:
             # Sync with model_path if provided, else use Config
        ...
        self._setup_processor_and_tokens(use_base_model=use_base_model)
  ...
  # Let transformers pick (SDPA/Flash)
    ...
    def _setup_processor_and_tokens(self, model_path: Optional[str] = None, use_base_model: bool = False):
        """
    ...
    def run_forward_with_grad(self, image_path, prompt_text, target_size=512):
        ...
        try:
            ...
            # correctly when image_token_index was changed (262144255999).
    ...
    def analyze_image(self, image_input, query="Analyze the image and output findings and bounding boxes in JSON format.", task="detection", modality=None, **kwargs):
        """
        ...
        if task == "detection":
            ...
            if MODALITY_PROMPTS_AVAILABLE and modality:
                ...
                # Baseline detection prompt (WITHOUT reasoning by default for existing models)
                # Refined with VinDr-CXR vocabulary to guide model and reduce hallucinations
                detection_prompt = (
                    ...
                    "Valid classes to map your findings to: Aortic enlargement, Atelectasis, Calcification, Cardiomegaly, Consolidation, Edema, Emphysema, Fibrosis, Infiltration, ILD, Lung opacity, Mass, Nodule, Pleural effusion, Pleural thickening, Pneumonia, Pneumothorax, Pulmonary fibrosis, Rib fracture.\n"
                    "IMPORTANT: Do not hallucinate. Prove what you see by strictly describing the visual pixels. The vast majority of images are NORMAL. Be extremely conservative.\n"
                    "If you are unsure or the image is ambiguous, return 'No significant abnormality'.\n"
                    "For each finding, provide the class name and specific bounding box [x1, y1, x2, y2] that tightly wraps the actual ROI in the image.\n"
                    "If no pathological findings are present, return a 'No significant abnormality' class with box [0,0,0,0].\n"
                    'Output JSON with compressed keys: "f", "c", "b", "r". Example: {"f": [{"c": "...", "b": [x1,y1,x2,y2], "r": "..."}]}'
                )
            
            # Inject reasoning requirement ONLY IF requested
            if kwargs.get("include_reasoning", False):
                # Update JSON format to include reasoning if not already there
                if '"reasoning":' not in detection_prompt:
                    # Target both formats (with or without existing description)
                    if '"description":' in detection_prompt:
                    ...
                    if "reasoning" not in detection_prompt.lower():
                        detection_prompt += " For each finding, provide a 'reasoning' field explaining the clinical evidence."
        ...
        if not json_str and ('findings' in decoded or '"f":' in decoded or '"c":' in decoded) and '{' in decoded:
            ...
            # Heuristic for clinical reasoning truncation
            if ('"r":' in repair or '"reasoning":' in repair) and repair.count('"') % 2 == 1:
                repair += '"'
        ...
        if task == "detection" and boxes:
            try:
                # Parse as findings and validate
                findings = self._extract_findings(decoded)
                
                # TEMPLATE COORDINATE FILTER: Discard boxes matching memorized prompt/training examples
                TEMPLATE_COORDS = [
                    [234, 567, 432, 789], [567, 234, 789, 432],  # xray example 1
                    [678, 123, 890, 456], [123, 678, 456, 890],  # xray example 2
                    [345, 678, 567, 890], [678, 345, 890, 567],  # mri example
                    [123, 456, 789, 901], [456, 123, 901, 789],  # ct example
                    [567, 123, 890, 432], [123, 567, 432, 890],  # ultrasound example
                    # Known hallucinations from retraining phase (found in raw_samples.json)
                    [738, 459, 861, 521],  # Pulmonary fibrosis / Bilateral reticular markings
                    [510, 311, 611, 411],  # Aortic enlargement / Prominent aortic knob
                    [119, 160, 194, 224],  # Pleural thickening
                    [410, 414, 631, 514],  # Cardiomegaly
                    [698, 574, 831, 634],  # Pleural thickening variant
                    [718, 574, 831, 634],  # Pleural thickening variant 2
                    [510, 394, 618, 472],  # Aortic enlargement variant
                    [685, 347, 911, 522],  # Pulmonary fibrosis variant
                    [190, 511, 231, 532],  # Nodule/Mass variant
                    [201, 412, 279, 472],  # Infiltration variant
                    [199, 670, 341, 754],  # Pleural effusion variant
                    [600, 338, 811, 456],  # Nodule/Mass variant 2
                    [685, 734, 798, 788],  # Pleural thickening variant 3
                ]
                filtered_findings = []
                for f in findings:
                    if not isinstance(f, dict): continue
                    box = f.get("box", f.get("b", []))
                    box_int = [int(b) for b in box] if len(box) == 4 else box
                    if box_int in TEMPLATE_COORDS:
                        print(f"Logic: Filtered memorized template coords {box_int}")
                        continue
                    filtered_findings.append(f)
                findings = filtered_findings
                
                # RELAXED CONCLUSION OVERRIDE: Only suppress when model detected ZERO real pathology
                # Previously this suppressed ALL findings when conclusion said "normal" —
                # even when the model correctly found pathology but hedged in its conclusion.
                conclusion_match = re.search(r'"conclusion":\s*"(.*?)"', decoded, re.IGNORECASE)
                has_real_findings = any(
                    isinstance(f, dict) and 
                    f.get("class", f.get("c", "")).lower() not in 
                    ["no significant abnormality", "no finding", "normal", "healthy"] and
                    f.get("box", f.get("b", [0,0,0,0])) != [0,0,0,0]
                    for f in findings
                )
                
                if conclusion_match and not has_real_findings:
                    conc = conclusion_match.group(1).lower()
                    if "no significant abnormality" in conc or "normal" in conc or "no acute" in conc:
                        findings = []  # Genuinely normal — clear findings
                
                if findings:
                    validated = VocabularyConstraint.validate_findings(findings)
                    
                    # Replace boxes with validated ones
                    boxes = [f.get("box", f.get("b", [])) for f in validated]
                    boxes = [b for b in boxes if len(b) == 4]
                    
                    # RESERIALIZE: Update decoded text with cleaned findings
                    if use_custom_stopping:
                        cleaned_data = {"findings": validated}
                        if conclusion_match:
                             cleaned_data["conclusion"] = conclusion_match.group(1)
                        decoded = json.dumps(cleaned_data)
    ...
    def _extract_reasoning(self, text: str) -> list:
        """Extract reasoning list from model output."""
        try:
            # Try parsing as JSON
            ...
            if json_match:
                ...
                if isinstance(f_list, list):
                    ...
                    for f in f_list:
                        if not isinstance(f, dict): continue
                        r = f.get("reasoning", f.get("r", ""))
                        if not r: # Fallback to description/d
                            r = f.get("description", f.get("d", ""))
                        res.append(r)
                    return res
            return []
        except:
    ...
    def _extract_findings(self, text: str) -> list:
        """Extract findings list from model output, handling XML-wrapped and plain JSON."""
        try:
            # STEP 1: Strip XML tags that wrap JSON output (from modality prompts)
            # Model may output: <reasoning>...</reasoning><findings>{"findings":...}</findings>
            cleaned = text
            # Remove <reasoning>...</reasoning> blocks (may contain { which confuses JSON regex)
            cleaned = re.sub(r'<reasoning>.*?</reasoning>', '', cleaned, flags=re.DOTALL)
            # Remove <findings>, </findings>, <output>, </output> XML wrappers
            cleaned = re.sub(r'</?findings>', '', cleaned)
            cleaned = re.sub(r'</?output>', '', cleaned)
            cleaned = re.sub(r'</?context>', '', cleaned)
            cleaned = cleaned.strip()
            
            # STEP 2: Try to parse JSON from cleaned text
            ...
            if json_match:
                ...
                if isinstance(data, dict):
                    ...
                    if "f" in data:
                        return data["f"]
                    # Support single finding dictionary directly
                    if "c" in data or "class" in data or "b" in data or "box" in data:
                        return [data]
            
            # STEP 3: Fallback — try original text if cleaning removed too much
            if cleaned != text:
                json_match = re.search(r'\{.*\}', text, re.DOTALL)
                if json_match:
                    data = json.loads(json_match.group(0))
    ...
    def _parse_boxes(self, text):
        ...
        try:
            ...
            if json_match:
                ...
                if isinstance(data, dict):
                        ...
                        if isinstance(raw_boxes, list):
                            for b in raw_boxes:
                                if isinstance(b, list) and len(b) == 4:
                                    # Coordinate Guard: Ensure values are 0-1000
                                    if all(0 <= x <= 1000 for x in b):
                                        boxes.append(b)

                    if "findings" in data:
                        findings = data["findings"]
                    elif "f" in data: # Support compressed key
                        findings = data["f"]
                    elif "c" in data or "class" in data or "b" in data or "box" in data:
                        # Direct single-finding dictionary
                        findings = [data]
                    else:
                        findings = None
                    ...
                    if findings and isinstance(findings, list):
                            for item in findings:
                                if isinstance(item, dict):
                                    for k in ["box", "bbox", "bbox_2d", "box_2d", "coordinates", "b"]:
                                        if k in item:
                                            b = item[k]
                                            if isinstance(b, list) and len(b) == 4:
                                                # Coordinate Guard
                    ...
                    for item in data:
                        if isinstance(item, dict):
                            for k in ["bbox", "bbox_2d", "box_2d", "box", "coordinates", "b"]:
                                if k in item:
                                    b = item[k]
                                    if isinstance(b, list) and len(b) == 4:
                                        boxes.append(b)
        ...
        # 5. Deduplicate nearly identical boxes (often produced during loops)
        if boxes:
            deduplicated = []
            for b in boxes:
                is_duplicate = False
                for existing in deduplicated:
                    # Exact match or extremely high overlap (IoU > 0.95)
                    if b == existing:
                        is_duplicate = True
                        break
                    
                    # Compute simple IoU for deduplication
                    try:
                        iou = self._compute_iou_internal(b, existing)
                        if iou > 0.95:
                            is_duplicate = True
                            break
                    except:
    ...
    def _compute_iou_internal(self, boxA, boxB):
        """Internal IoU for normalized boxes [x1, y1, x2, y2]."""
    ...
    def unload(self):
        """Zero-out model for memory safety."""
        print("Unloading MedGemma...")
        if self.model:
            del self.model
            self.model = None
        if self.processor:
